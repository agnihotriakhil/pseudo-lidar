...
ROI = 50        # Region of interest side length, in meters.
...
#    To remove previous files and prevent overlap
    ...
    for head, tail, left, right in zip(heads, tails, lefts, rights):
    	...
    	img = img.rotate(180)
    	img.convert('RGB').save("../convert_image/{0}/Pointcloud_images/{1}.png".format(lidar,name))
    	np.savetxt("../convert_image/{0}/Pointclouds/{1}.bin".format(lidar,name),point_cloud.tolist())
    
    # Copying labels for processing 
    for label in glob.glob('../{}_datacollected_[l]*/*'.format(lidar)):
        try:
